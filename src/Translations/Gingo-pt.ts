const PROJECT_DESC = `Gingo é um aplicativo de delivery e chat em tempo real, onde os usuários podem conversar a qualquer momento com estabelecimentos ou entre sí.
...
const CHALLENGE = `Meu maior desafio foi com a construção do chat, tanto no back quanto no frontend, devido a sua construção offline first no frontend e seu sistema de garantir que nenhuma mensagem seja perdida devido a oscilação de internet.
...
const LEARNING = `Com esse projeto aprendi a usar express e socket no backend com Node. 
